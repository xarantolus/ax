...
class JumpTestCase:
	...
	def __init__(self, set_flags: List[str], flags_not_set: List[str], initial_rip: int, final_rip: int, initial_bytes: List[str], final_bytes: List[str], padding: int):
	...
	def create(initial: str, padding: int, final: str):
		with tempfile.TemporaryDirectory(prefix="ax_jumper_", dir="/dev/shm") as tmpdir:
			...
			return JumpTestCase(set_flags, flags_not_set, initial_rip, final_rip, initial_bytes, final_bytes, padding)

	def __str__(self):
		return f"""
		.set_flags = {self.set_flags},
		.flags_not_set = {self.flags_not_set},
		.initial_rip = {self.initial_rip},
		.final_rip = {self.final_rip},
...
if __name__ == '__main__':
	if len(sys.argv) == 1:
		sys.argv += ["mov rax, 5; JMP .label", "50", ".label: cmp rax, 5",]
	...
	print(JumpTestCase.create(code_start, padding, code_end))
