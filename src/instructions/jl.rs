...
impl Axecutor {
    pub fn mnemonic_jl(&mut self, i: Instruction) -> Result<(), AxError> {
        ...
        match i.code() {
            ...
            _ => panic!("Invalid instruction code {:?} for mnemonic Jl", i.code()),
        }
    }
    ...
    fn instr_jl_rel8_64(&mut self, i: Instruction) -> Result<(), AxError> {
        ...
        if (self.state.rflags & FLAG_SF == 0) != (self.state.rflags & FLAG_OF == 0) {
            match i.op0_kind() {
                ...
                _ => panic!("Invalid op0_kind {:?} for JL rel8_64", i.op0_kind()),
            }
        }
    ...
    fn instr_jl_rel32_64(&mut self, i: Instruction) -> Result<(), AxError> {
        ...
        if (self.state.rflags & FLAG_SF == 0) != (self.state.rflags & FLAG_OF == 0) {
            match i.op0_kind() {
                ...
                _ => panic!("Invalid op0_kind {:?} for JL rel32_64", i.op0_kind()),
            }
        }
